...
\begin{itemize}
  \item A pre-registered vision-loop twin in which the robustness claim is \emph{untestable under our precondition} --- the deciding arm never qualifies, and we hold to zero OOD evaluations (C1).
...
\paragraph{Protocol economics (C7a).} The binding run comprised \textbf{13 full training cycles} (plus one GRU control), $\approx$\textbf{57.1\,h} of cumulative compute (sum of per-cycle wall time; parallel wall-clock was shorter but is not an authoritative per-arm cost). Arithmetic early resolution spared a complete four-seed batch relative to the 40 cycles a full $n{=}10$ over four legs would have required.
...
\begin{itemize}
  ...
  \item \textbf{No OOD measurement in the vision-loop regime.} This is a \emph{consequence} of the unmet precondition, not a design choice.
...
We release the liquidflight v1.0 and liquidsight repositories (tags and gate commits), the frozen documents (F3\_PRE0, decisions and Annexes 1--4, P-SANITY, F3-GATE), sha256 manifests of the execution layer, the seed pools, the per-cycle logs, and the reports. The companion P0 ships as a separate PDF/arXiv artifact \citep{zydziak2026p0}. Instrument determinism is within-machine bit-exact; bit-for-bit reproduction across different GPUs is not guaranteed (inherent to CUDA).
