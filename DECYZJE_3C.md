...
| **D2′** | tryb R-B (S3c1-R) | **admisja NA WEJŚCIU (jednorazowa) + sufit twardy 6.0 s** | Bug-fix (`ANEKS_3C1_SEMANTYKA.md`): iter-1 stosowała θ_age jako predykat ciągły → 83/100 odmów STALE na czystej bazie (sukces 67→9%). PRE_3C0 §2 mówi „admisja wejścia w dwell" (kontrola raz przy przekroczeniu 0.5 m); po admisji dwell biegnie bez kontroli age poniżej sufitu 6.0 s (G2: zero sukcesów przy age>6 s). θ_age/T_hold niezmienione. |
